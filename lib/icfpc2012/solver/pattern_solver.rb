...
module Icfpc2012
  class PatternSolver < LocalSolver
    def initialize(pattern_defs, map, region, target)
      super(map, region, target)
        ...
        (@region.x1..max_x).each do |map_x|
          (@region.y1..max_y).each do |map_y|
            if pattern.match?(@map, map_x, map_y)
              exit_cell = @target.target_cells.detect{ |cell| pattern.cell_is?(cell[0]-map_x, cell[1]-map_y, 'x') }
